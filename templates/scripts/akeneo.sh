...
akeneo_exec_in_akeneo_dir() {
	docker exec -it "$1" /bin/bash -c "cd $AKENEO_INST_DIR && ${*:2}"
}

# Download Akeneo installation files using composer
print_green "Akeneo files were succesfully downloaded."
akeneo_exec /bin/bash -c "sudo chown -R $AKENEO_USER:$AKENEO_USER $AKENEO_INST_DIR"
akeneo_exec /bin/bash -c "composer create-project akeneo/pim-community-standard $AKENEO_INST_DIR '7.0.*@stable'"

# copy .env to .env.local
print_green "Akeneo environmental file was created."
akeneo_exec_in_akeneo_dir /bin/bash -c "cp .env .env.local"

# create an executable akeneo console application
akeneo_exec_in_akeneo_dir /bin/bash -c "chmod u+x bin/console"

# append details to .env.local file
print_green "Appending details to the Akeneo .env.local file"
akeneo_exec_in_akeneo_dir /bin/bash -c "
...
akeneo_exec_in_akeneo_dir /bin/bash -c "yes | npx update-browserslist-db@latest"

# Install akeneo
print_blue "Installing Akeneo"
akeneo_exec_in_akeneo_dir /bin/bash -c "NO_DOCKER=true make dev"

print_green "Creating User $AKENEO_USERNAME"
akeneo_exec_in_akeneo_dir /bin/bash -c "bin/console pim:user:create $AKENEO_USERNAME $AKENEO_PASSWORD $AKENEO_USER_EMAIL $AKENEO_FIRST_NAME $AKENEO_LAST_NAME $AKENEO_LOCALE --admin -n --env=dev"
